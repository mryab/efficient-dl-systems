...
def test_http_endpoint(http_host, eval_data):
    predict_url = str(furl(http_host) / "predict")
    scores = []
    for img_name, labels in eval_data.items():
        img_url = "https://storage.yandexcloud.net/effdl2022-coco/{}".format(img_name)
...
def test_grpc_endpoint(grpc_host, eval_data):
